...
# 随机产生车牌号
def carNumber():
    k=str(fake.license_plate())
    M=k.replace(" ", '').replace('-','').replace('I','1').replace('O','0').replace('•','0')[0:5].replace(' ',random.choice(choices))
    if len(M) == 4: M = M + random.choice(choices)
    return random.choice(ctiy)+M
