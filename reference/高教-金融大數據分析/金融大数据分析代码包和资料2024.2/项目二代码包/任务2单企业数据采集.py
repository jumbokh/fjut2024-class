...
code = [("600000", "浦发银行")]
year = ["2021"]
report_period_id = ["4000"]  
url = ["https://ssecurity.seentao.com/debug/security/security.info.get"]

# 三、使用pandas库文件打开文件
name_file = pd.read_excel('数据采集/中英文对照表/中英指标对照.xlsx', sheet_name='sz_info')
...
try:
    ...
    end_data.to_excel('浦发银行2021年1季度基本信息数据采集结果.xlsx', encoding='utf-8', index=False)  
except Exception as e:
    print('采集失败', e)
